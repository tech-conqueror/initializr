...
import static com.techconqueror.initializr.core.tier.RepositoryGenerator.generateRepository;
import static com.techconqueror.initializr.core.tier.RestControllerGenerator.generateController;
import static com.techconqueror.initializr.core.tier.ServiceGenerator.generateService;
...
public class HibernateEntityGenerator {
     ...
     */
    public static void generateEntitiesForAllTables(String packageName, String outputPath, Connection connection)
            throws Exception {
    ...
    private static void generateEntityFromTable(
            String packageName, String tableName, String outputPath, Connection connection) throws Exception {
        ...
        generateController(packageName, metadata, outputPath);
    }
    ...
    private static void generateEntityFile(EntityClassMetadata metadata, String outputPath) throws IOException {
        TypeSpec entityClass = ClassSource.createClass(metadata.getName())
                ...
                .build();
        JavaFile javaFile =
                JavaFile.builder(metadata.getPackageName(), entityClass).build();
     ...
     */
    private static EntityClassMetadata fetchTableMetadata(String packageName, String tableName, Connection connection)
            throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(columnQuery)) {
            ...
            while (resultSet.next()) {
                ...
                TypeName fieldType = isForeignKey
                        ? mapToEntityClass(packageName, referencedTable)
                        : TypeName.get(mapToJavaType(dataType));
    ...
    private static EntityFieldMetadata createRelationshipField(
            String packageName, String parentTable, String childTable, boolean isUnique) {
        ...
        var fieldType = isUnique
                ? mapToEntityClass(packageName, childTable)
                : ParameterizedTypeName.get(ClassName.get(Set.class), mapToEntityClass(packageName, childTable));

        // Select the appropriate annotation
        AnnotationSpec.Builder annotation = isUnique
     ...
     * <p>
     * This method takes the name of a referenced table in the database and converts it
     * to the appropriate Java entity class name by capitalizing the table name and
     * converting it from snake_case to UpperCamelCase. The resulting class name is
     * used to reference the related entity in the generated code.
     * </p>
     *
     * @param packageName     the package name to assign to the generated class.
     * @param referencedTable the name of the referenced table in the database (in snake_case).
     * @return a {@link ClassName} representing the corresponding Java entity class for the referenced table.
