 ...
 *
 * <p>This class provides methods to construct Java class structures dynamically, including
 * annotations, fields, getter and setter methods, and optional constructors. It leverages the
 * {@code TypeSpec.Builder} from JavaPoet to define and build classes programmatically.
 */
public class ClassSource {
  ...
  /**
   * Adds a list of fields to the class. For each field, getter and setter methods are also
   * generated.
   *
  ...
  private static FieldSpec createField(FieldMetadata fieldMetadata) {
    return FieldSpec
      .builder(fieldMetadata.getTypeName(), fieldMetadata.getName(), Modifier.PRIVATE)
  ...
  private static Iterable<MethodSpec> createGetterSetterMethods(FieldMetadata fieldMetadata) {
    MethodSpec getter = MethodSpec
      .methodBuilder("get" + capitalize(fieldMetadata.getName()))
      .addModifiers(Modifier.PUBLIC)
    ...
    MethodSpec setter = MethodSpec
      .methodBuilder("set" + capitalize(fieldMetadata.getName()))
      .addModifiers(Modifier.PUBLIC)
